...
Environment variables:
    FCB_HOME          Storage root directory       (default: ~/.fatcatbrain)
    FCB_LLM           "fake" or "ollama"           (default: fake)
    FCB_OLLAMA_MODEL  Ollama model name            (default: llama3.1)
    FCB_OLLAMA_HOST   Ollama host URL              (default: ollama's own default)
    FCB_PROJECT       Active project id            (default: "default")
    FCB_TRANSCRIPTS_DIR  Dir to watch in listen mode (default: none)
    FCB_MIN_CONFIDENCE   Min candidate confidence in listen mode (default: 0.6)
    FCB_LISTEN_INTERVAL  Poll interval seconds in listen mode    (default: 10)
"""
...
class Settings:
    ...
    def from_env(cls, env: dict[str, str] | None = None) -> "Settings":
        ...
        transcripts_raw = pick("FCB_TRANSCRIPTS_DIR", "transcripts_dir", None)
        return cls(
            home=home,
            llm=str(pick("FCB_LLM", "llm", "fake")).strip().lower(),
            ollama_model=str(pick("FCB_OLLAMA_MODEL", "ollama_model", DEFAULT_OLLAMA_MODEL)),
            ollama_host=pick("FCB_OLLAMA_HOST", "ollama_host", None) or None,
            project_id=str(pick("FCB_PROJECT", "default_project", detect_project_id())),
            transcripts_dir=(
                Path(str(transcripts_raw)).expanduser() if transcripts_raw else None
            ),
            min_confidence=_as_float(
                str(pick("FCB_MIN_CONFIDENCE", "min_confidence", "")) or None,
                DEFAULT_MIN_CONFIDENCE,
            ),
            listen_interval=_as_float(
                str(pick("FCB_LISTEN_INTERVAL", "listen_interval", "")) or None,
