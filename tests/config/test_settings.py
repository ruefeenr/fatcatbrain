...
def test_defaults_when_no_config(tmp_path: Path):
    s = Settings.from_env({"FCB_HOME": str(tmp_path / "home")})
    assert s.llm == "fake"
...
def test_config_file_values_are_used(tmp_path: Path):
    ...
    s = Settings.from_env({"FCB_HOME": str(home)})
    assert s.llm == "ollama"
...
def test_env_overrides_config(tmp_path: Path):
    home = tmp_path / "home"
    _write_config(home, {"llm": "ollama", "ollama_model": "qwen2.5"})
    s = Settings.from_env({"FCB_HOME": str(home), "FCB_LLM": "fake"})
    assert s.llm == "fake"
...
def test_slugify_project():
    ...
    assert slugify_project("fatcatbrain") == "fatcatbrain"
...
def test_env_project_overrides_detection(tmp_path: Path):
    s = Settings.from_env(
        {"FCB_HOME": str(tmp_path / "home"), "FCB_PROJECT": "explicit"}
    )
